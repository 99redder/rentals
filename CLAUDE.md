...
### 2026-08-12 — Tax Planning projection: 6AL sale proceeds + 4781MC cash-to-close interest; repair line removed
...
- **Also adds a full-calendar-year of interest on the 4781MC settlement cash-to-close** (`tpProjectedCashToCloseInterest`), auto-derived from `constructionCashToClose('4781MC')` (the "4781MC Cash to Close" Cash Flow line) and zeroed once the build settles. Deliberately assumes the cash sits in checking all 12 months (× 12/12) rather than modeling the settlement date — a small, conservative simplification. Overridable via `projection_rh_cashtoclose` (blank = auto). Added additively on top of the base checking balance; if that cash is already inside the live checking balance, zero the override to avoid double-counting.
